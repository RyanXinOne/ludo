...
Firework.prototype.draw=function(){
	ctxx.beginPath();
	//轨迹的起点
	ctxx.moveTo(this.guiji[this.guiji.length-1][0],this.guiji[this.guiji.length-1][1]);
	//绘制线条到目标点
	...
	//画出目标小圆
	ctxx.beginPath();
	ctxx.arc(this.tx,this.ty,this.targetRad,0,Math.PI*2);
	ctxx.stroke();
}
//定义烟花碎屑方法
...
function loop(){
	...
	ctxx.fillRect(0,0,cc.width,cc.height);
	ctxx.fillStyle='rgb(0,0,0,0.5)';
	//显示源图像和目标图像。
	...
	{
		fireworks.push(new Firework(cc.width/2,cc.height,random(0,cc.width),random(0,cc.height)));
		timerTick=0;
	}
	else{
		timerTick++;
...
function cellebrate() {
	cc = document.getElementById("firework");
	cc.width = document.documentElement.clientWidth-10;
	cc.height = document.documentElement.clientHeight-10;
	ctxx=cc.getContext("2d");
	loop();
}